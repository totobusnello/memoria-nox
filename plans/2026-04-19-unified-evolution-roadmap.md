...
| **1.7b-c** | **Compiled truth + timeline format** | 🔧 FOUNDATION DONE (2026-04-23) | ~40min foundation (vs 1-2d full) | 1.7b-b | **Foundation shipped**: schema v10 (section TEXT + section_boost REAL), src/ingest-entity.ts parser (3-section: frontmatter/compiled/timeline, SECTION_BOOST={compiled:2.0, frontmatter:1.5, timeline:0.8}), getSectionDistribution em /api/health, 2 entities piloto (agents/nox.md + systems/nox-mem.md) = 20 chunks (2 compiled, 2 frontmatter, 16 timeline). Compiled truth aparece como #1 em hybrid search (score 32.79) pra queries "current state". `<!-- retention: never -->` via frontmatter funcionou — never_decay subiu 84→92. **Pendente pra close Fase 1.7b**: migração massiva de memory/projects.md, memory/decisions.md, memory/lessons.md → entities/ (precisa curadoria manual + /memory-recompile skill). Agendar sessão dedicada (4-6h). |
