...
| **1.7b-c** | **Compiled truth + timeline format** | 🔧 FOUNDATION DONE + CLI (2026-04-24) | ~40min foundation + 15min CLI | 1.7b-b | **Foundation shipped (2026-04-23)**: schema v10 (section TEXT + section_boost REAL), src/ingest-entity.ts parser (3-section: frontmatter/compiled/timeline, SECTION_BOOST={compiled:2.0, frontmatter:1.5, timeline:0.8}), getSectionDistribution em /api/health, 2 entities piloto (agents/nox.md + systems/nox-mem.md) = 20 chunks. Compiled truth aparece como #1 em hybrid search pra queries "current state". **CLI formal adicionado (2026-04-24)**: `nox-mem ingest-entity <file>` registrado em src/index.ts (10 linhas, backup em .bak-20260424-115355). Substitui o hack `node -e "import(...)"`. Re-ingest validou 8+12 chunks. **Pendente pra close Fase 1.7b**: migração massiva memory/projects.md + memory/decisions.md + memory/lessons.md → entities/ (curadoria manual + /memory-recompile skill). Agendar 4-6h dedicada. |
