...
| 2 | Graphify + GitHub repos | ✅ DONE (9/9 com código, 2026-04-23) | — | IM + 1.7b-a | **9 repos indexados** via graphify update (AST-only, zero custo LLM): sao-thiago-fii (94), projeto-ai-galapagos (147), Granix-App (163), daily-tech-digest (112), nox-supermem (56), agent-hub-dashboard (240), memoria-nox (50), Future-Farm (34), GalapagosApp (150) = **1046 graph_node chunks** (todos retention=60d). **8 projetos docs-only** (Frooty, Superfrio, GranCoffee, BIolab AI, Curso-AI, Fake-News-Check, Area-Manuel_Nobrega, Area-Campolim-Sorocaba) retornaram "no code files" no `graphify update` — já cobertos pelo IM (docs/*.md ingestados como team/daily chunks). Hybrid search mistura code+docs+rationale no top 3. Exit criteria atingido (100% dos repos com código). Full-LLM extraction (Claude Vision) via skill pode agregar em sessão futura pros docs-only. |
| **1.7b-b** | **Salience formula formal** | ✅ DONE shadow-mode (2026-04-23) | ~45min (vs 4h est) | 2 | Schema v9 (pain REAL DEFAULT 0.2) + src/salience.ts (novo) + /api/health.salience expondo promote/retain/review/archive buckets + mean/median. **Shadow-mode**: calcula mas não aplica no ranking/tier (NOX_SALIENCE_MODE=shadow default). Baseline 2026-04-23: 207 promote_candidates, 1886 archive_candidates (~26%), median=0.16. Backfill em 7347 chunks: pain=1.0 (256 prod-outage/crash), pain=0.8 (43 lesson), pain=0.5 (469 bug/error), pain=0.3 (105 warn/deprec), pain=0.2 (6474 default). Importance re-infered: 17 feedback=1.0. Ativação via `NOX_SALIENCE_MODE=active` após ≥1 semana de observação. |
| **1.7b-c** | **Compiled truth + timeline format** | ✅ DONE (2026-04-24) | ~3h total | 1.7b-b | **Foundation (2026-04-23)**: schema v10 (section TEXT + section_boost REAL), src/ingest-entity.ts parser, 2 entities piloto = 20 chunks. **CLI (2026-04-24 manhã)**: `nox-mem ingest-entity <file>` registrado. **Migração massiva (2026-04-24 tarde)**: 12 projects + 42 lessons + 127 decisions = 181 entities, 712 chunks via scripts/migrate-*.py. sectionDistribution compiled 2→183 (+181), frontmatter 2→183, timeline 16→366. **Search section_boost shadow-mode (src/search.ts)**: env var NOX_SECTION_BOOST_MODE (shadow default \| active), helper applySectionBoost, telemetria NOX_SECTION_BOOST_LOG=1. A/B coletado: delta sutil no ranking (boost 2.0 em compiled ainda é modesto vs BM25 legacy). **Skill /memory-recompile** (~/Claude/skills/memory/memory-recompile/): Gemini Flash-Lite reescreve compiled baseado em timeline; user-triggered, dry-run mode, timeline nunca editado. **Pendências pós-close**: 7d observação shadow → decisão de ativação + arquivamento dos 3 source files (memory/projects.md, decisions.md, lessons.md). |
