...
**Os dois papers, e por que são vision e não sprint.** O Paper 1 é o argumento de **qualidade**, e ele foi **recusado** pelo arXiv em 2026-09-03 (`submit/7771319`). A rota escolhida em 2026-09-10 era **melhorar o manuscrito e apelar** — e foi **invertida em 2026-09-12** para **TMLR primeiro**, apelação depois (ver `docs/ESTADO-DOS-PAPERS-2026-09-12.md`) — com duas restrições registradas de propósito, porque quem ler isto depois precisa saber que foram vistas e não ignoradas: o email de recusa nomeia publicação em journal convencional como a condição da reconsideração, e a apelação é de **uso único** — negada, é permanente. O que sustenta a escolha é uma assimetria medida: **0 de 12** papers de agent-memory no arXiv têm `journal_ref` ou `doi`, isto é, nenhum passou por journal; mas todos foram aceitos na primeira submissão, então a condição nunca lhes foi aplicada. O gap que resta no manuscrito é de **forma**, não de rigor — e depois dos consertos de 09-10 ele é **um só**: comprimento. ⚠️ Estes números envelheceram duas vezes no mesmo dia. **Medido em 2026-09-12 ~18h**, com o controle positivo passando: **25.671 palavras (1,49× o mais longo dos aceitos)** e **2,14 refs/mil** contra piso **2,06**, com **55 obras**. Uma medição de 12-09 à tarde dava 27.489/1,59×/2,07 e era anterior ao #538, que recontou o `bibitem-census.json`. Nunca citar estes três de memória — rodar `paper/publication/scripts/reconta-regua-nosso-lado.py`, que sai `!=0` se não reproduzir. A densidade já foi dada como fechada em 2,078 e não estava: o numerador contava footnotes, e quatro obras apareciam duas vezes — o valor real era 1,937, abaixo do piso. Fechou de verdade movendo 2.177 palavras de histórico de ablação para o suplemento. O Paper 2 é o argumento de **método** — mede o efeito de intervir na própria memória, com desenho pré-registrado, e a contribuição é o desenho: no N realizado o efeito de interesse não tem resultado possível (o MDE satura em 100%), e dizer isso em vez de esconder é o ponto. Os dois existem porque credibilidade técnica é o ativo do projeto, e essa é a mesma razão pela qual `COMPARISON.md` não sai quando perdemos.

**2026 Q4 → 2026-11 — Paper 1 encurtado e apelado, e então GTM.** Ordem decidida em 2026-09-10: (1) encurtar o §5 do Paper 1 (o gap de forma que resta é comprimento), (2) ~~apelar no arXiv — não submeter ao TMLR primeiro~~ ⇒ **invertido em 2026-09-12: submeter ao TMLR primeiro**, e apelar depois com o status em mãos, (3) **GTM Phase 2 em novembro** — depois do paper, não antes —, (4) Paper 2 como method paper em paralelo, com o desfecho do ensaio fechado em 20/09. Product breadth no Tier A (P1 `answer` já em prod, P2 hooks, P3 temporais, P4 connect Tier A, P5 viewer) continua sendo o alvo do pilar P, mas atrás dessa fila: três IDEs profundos > doze adapters rasos, e nenhum deles antes de haver um paper que justifique a visita ao repo.
...
*Última revisão: 2026-09-12 (v16 + errata: ordem TMLR/apelação invertida, régua re-medida).
*Revisão anterior: 2026-09-10 (v16 — marcos reconciliados com o real + os dois papers).* Próxima revisão: pós-apelação do Paper 1 no arXiv, ou pós-GTM Phase 2 (nov/2026), ou sempre que pivot estratégico justificar.*
