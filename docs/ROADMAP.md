...
| **E03b** | — | **A6 activate HOLD 2026-05-16** — gate review descobriu consumer absent (336/336 logs shadow = canary; 0 agentes Discord usam `/api/search`). Código funciona quando exercitado mas sem traffic real. **Pré-req ACTIVATE: task #18** (integrar `getVaultFacts()` em `nox-mem search` CLI ou agente Discord, 1-2h). Decisão D37: não CUT (hipótese valor sólida), não ACTIVATE (sem evidência consumer). | 🚫 HOLD (consumer absent) | 0.2 + 1-2h integração | task #18 integração + ≥1 consumer real |
...
| **E05b** | §11 Wave 1 | **Reason-aware Ranking Boost (E05 Phase 2)** — `kg_relations.relation_reason` vira sinal aditivo de retrieval. Schema v13 (search_telemetry +3 cols). `src/lib/reason-boost.ts` ~270 LOC, 10/10 tests. **Gate review 2026-05-16 (re-executado manualmente após cron silent-fail 13/05):** Round 2 verdict **KEEP-SHADOW** — cross-agent +0.0765 ✅ (resolveu via kg-extract focado --limit 100, +538 relations) mas procedure regrediu -0.0503 ❌ (qid=52 1.0→0.63 sozinha; n=9 underpowered). **Diagnóstico arquitetural:** falta de poder estatístico no golden set (n=4-9 por categoria) → kg-extract MOVE qual categoria regride, não resolve globalmente. **Decisão:** aguardar golden set expansion n≥30 (E14 pré-req semana 20-23/05), re-rodar gate em ~2 semanas. Side-fixes (mesma sessão): script chmod +x, parser bug `json_object→json_group_object`, trap Discord alert pra exit≠0 (previne silent-fail futuro). Spec `specs/2026-05-06-E05b-reason-ranking-boost.md` §Gate review history. | 🟡 SHADOW round 2 (verdict 16/05: HOLD até n≥30) | 3h impl + 1h gate-fix + ~40min forense | re-trigger ~27 mai pós golden-set expansion |
