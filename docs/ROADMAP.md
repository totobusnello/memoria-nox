...
| **E05b** | §11 Wave 1 | **Reason-aware Ranking Boost (E05 Phase 2)** — `kg_relations.relation_reason` vira sinal aditivo de retrieval. Pesos: depends_on/replaces=0.15, extends=0.12, derived_from=0.10, opposes=0.08, mentions=0.03, unknown=0. Cap 0.30 single-pass (regra #5). Schema v13 (search_telemetry +3 cols `reason_boost_applied/reason_relations_used/reason_boost_mode`). `src/lib/reason-boost.ts` ~270 LOC, 10/10 tests. Shadow active VPS 2026-05-06 19:48 BRT (`NOX_REASON_BOOST_MODE=shadow`). **Achado:** cobertura `evidence_chunk_id` ~0.47% (291/61285 chunks) — kg-extract loop tmux rodando target 3000. Spec `specs/2026-05-06-E05b-reason-ranking-boost.md` | ✅ DONE shadow | 3h (real ~2.5h) | gate 2026-05-13: Δ entity ≥+3pp + Δ cross-agent ≥+3pp + strong cats não regridem >1pp |
| **E13** | §11 Wave 1 | **Temporal-aware Ranking** — detector regex pra queries temporais (`quando`, `primeira/última`, `deployado`, ISO date) + override `section_boost`: timeline 0.8→1.4, compiled 2.0→1.0, frontmatter 1.5→0.9. Aplicado SOMENTE em mode=active + isTemporal. Substitui lookup (não soma — single-pass aditivo). Schema v14 (search_telemetry +2 cols `was_temporal_query/temporal_boost_mode`). `src/lib/temporal-detector.ts` ~110 LOC, 21/21 tests + suite 140/141 (1 skip pré). Shadow active VPS 2026-05-06 20:33 BRT (`NOX_TEMPORAL_BOOST_MODE=shadow`). **Root cause:** 100% gold temporal cured (Q70/Q71) em section=timeline. **Side-quest:** 27% gold corpus vazio (16/60 queries) — Toto curar durante shadow window. Spec `specs/2026-05-06-E13-temporal-aware-ranking.md` | ✅ DONE shadow | 2h impl + 1h Toto cure | gate 2026-05-13 (alinhado E05b): Δ temporal cured-only ≥+0.10 + Δ não-temporal ≥-0.005 + 0 timeouts |
