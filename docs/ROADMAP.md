...
| **Backbone Matrix bench (in-flight)** | Claude Sonnet 4.6 + Opus 4.7 + gpt-5 (if avail) + Gemini-3-flash × Phase H v2 baseline × 5-batch. Frontier reasoning over our retrieval pipeline. | **🟢 In-flight** | Cheapest F_MH lever if frontier backbone provides large reasoning lift. Budget $12. |
| **LoCoMo bench (in-flight)** | Cross-bench validation — nox-mem on LoCoMo paper baseline. Build harness + 100q smoke + full bench. | **🟢 In-flight** | Saturate winning evidence base vs published competitors (LangMem/MemoryBank/MemGPT/Mem0). Budget $7. |
