...
|---|---|---|---|
| **D01** | Q5 Cross-encoder reranker — **TRIGGER ANTECIPADO 2026-05-07**. Spec `specs/2026-05-07-D01-cross-encoder-reranker.md`. BGE-reranker-base via @xenova/transformers ONNX local (266MB quantized). Schema v16 (search_telemetry +6 cols). 16/16 tests pass. **Shadow mode ACTIVE** desde 2026-05-07 13:48 BRT (env `NOX_RERANKER_MODE=shadow`, top_k_in=50, top_k_out=10, model `Xenova/bge-reranker-base`). Smoke validado: row 2489 mode=shadow, position_changes=18, lift_score=0.656, latency cold 15.6s/warm 384ms. **Gate review eligible 2026-05-14** (7d shadow timer started). Activate criteria: Δ nDCG@10 ≥ +0.03 OR Δ MRR ≥ +0.05; latência p95 < 500ms | 🟡 SHADOW (7d gate ETA 2026-05-14) | 4h impl + 7d wall | nDCG 0.5831 < 0.6 motivou impl antecipada (closer to threshold than expected) |
