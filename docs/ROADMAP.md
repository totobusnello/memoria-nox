...
| **E12** | §11 (Tier 3) | Tier 3 OCR — pipeline cloud Google Document AI implementado 2026-05-07 (foundation: schema v15, ocr-detector + ocr-jobs + ocr-engine-stub + Tesseract + GoogleDocAi + CLI + routeIngest hook). Spec `specs/2026-05-07-E12-tier3-ocr.md`. **Batch full ⏳ ROLANDO desde 2026-05-07 13:37 BRT** (3005 files candidatos, ~$15-20 ETA, ~3-4h wall-clock) | 🔄 IN-PROGRESS (batch) | 6h impl + 4h batch | foundation done; cloud GoogleDocAi via SA `nox-mem-ocr-sa` configured |
...
| **E05b** | §11 Wave 1 | **Reason-aware Ranking Boost (E05 Phase 2)** — `kg_relations.relation_reason` vira sinal aditivo de retrieval. Schema v13 (search_telemetry +3 cols). `src/lib/reason-boost.ts` ~270 LOC, 10/10 tests. **Gate preview 2026-05-06:** KEEP-SHADOW (4/5 critérios falham — boost regride 4/6 cats). **Round 2 (2026-05-06 21:18 BRT):** pesos cortados pela metade via `NOX_REASON_BOOST_WEIGHTS_OVERRIDE` env (depends_on/replaces 0.15→0.075, extends 0.12→0.06, derived_from 0.10→0.05, opposes 0.08→0.04, mentions 0.03→0.015) + `NOX_REASON_BOOST_CAP=0.15`. Aguarda gate oficial 05-13 com mais sample + cobertura kg-extract. Spec `specs/2026-05-06-E05b-reason-ranking-boost.md` | 🟡 SHADOW round 2 (tuned) | 3h impl + tune | gate auto 2026-05-13 09:00 BRT via cron + script gate-review-e05b-e13.sh |
...
| **R02** | §11 Wave 3 | **Paper v1.1 ✅ DONE 2026-05-07** — Run #30/#31/#32 numbers (n=60 R01c-v1.1 post-cure): hybrid 0.5831±0.0046, FTS 0.0000, Δ +58.3pp, 4.0× BM25 Pyserini. §6.5 ablations E7-E9 confirmadas (E7=-0.029 marginal, E8=-0.041, E9=-0.032). §7 framing "Measurement instrument matures alongside the system" justificando +11.9% gain via gold cleanup + corpus growth (sem code change). Disclosure v1.0→v1.1 com v1.0.0 git tag preserved. PDF 891KB / 31 pages compilado. arxiv-submit-metadata + RESUMO + blog-post sincronizados. Pendente: arXiv submit 06-02. | ✅ DONE v1.1 | 5-6 (floor cognitivo) | submit ready 06-02; D01 reranker → v1.2 paper revision pós-7d gate |
...
|---|---|---|---|
| **D01-v1** | Q5 Cross-encoder reranker — **TRIGGER ANTECIPADO 2026-05-07 → CUT 2026-05-08**. Spec `specs/2026-05-07-D01-cross-encoder-reranker.md`. BGE-reranker-base via @xenova/transformers ONNX local (266MB quantized). Schema v16 (search_telemetry +6 cols). 16/16 tests pass. Shadow ATIVO 2026-05-07 13:48 BRT, gate eligible 2026-05-14 mas testado offline antecipado em 2026-05-08 12:20 BRT: **nDCG 0.3718 vs baseline 0.5831 → Δ = -0.2113 (CUT)**. Root cause: BGE-reranker-base inglês não transfere PT-BR domain. NOX_RERANKER_MODE=off restaurado em .env. **Aprendizado:** shadow lift_score 0.341 parecia promissor mas é métrica enganosa — sempre validar via offline nDCG antes de promover. **D01-v2 candidato deferred:** multilingual reranker (BGE-v2-m3) ou Cohere Rerank | ⛔ CUT v1 / 🤔 v2 candidate | 4h impl + 1h offline eval | nDCG 0.5831 baseline mantido sem reranker |
