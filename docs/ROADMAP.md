...
| **E05b** | §11 Wave 1 | **Reason-aware Ranking Boost (E05 Phase 2)** — `kg_relations.relation_reason` vira sinal aditivo de retrieval. Pesos: depends_on/replaces=0.15, extends=0.12, derived_from=0.10, opposes=0.08, mentions=0.03, unknown=0. Cap 0.30 single-pass (regra #5). Schema v13 (search_telemetry +3 cols `reason_boost_applied/reason_relations_used/reason_boost_mode`). `src/lib/reason-boost.ts` ~270 LOC, 10/10 tests + suite 119/120 (1 skip pré-existente). Shadow active VPS 2026-05-06 19:48 BRT (`NOX_REASON_BOOST_MODE=shadow`). **Achado:** cobertura `evidence_chunk_id` ~0.47% (291/61285 chunks) — sample shadow limitado; rodar `kg-build incremental` pré-gate. Spec `specs/2026-05-06-E05b-reason-ranking-boost.md` | ✅ DONE shadow | 3h (real ~2.5h) | gate 2026-05-13: Δ entity ≥+3pp + Δ cross-agent ≥+3pp + strong cats não regridem >1pp |
