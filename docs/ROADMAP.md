...
| **Wave C Composability triples** *(NEW POST-WAVE-B PRIORITY)* | Triple KG+MQ+MAP (validate full pipeline-stage additivity) + KG+MAP+temporal (3 stages) + Profile-chunk identification (close remaining MA -5.02pp gap) | **🟡 Queued** | Wave B validated orthogonal-stages hypothesis; Wave C tests scale. Predicted F_MH 33-40% MemOS closure. |
