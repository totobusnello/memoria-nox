...
> **Matched, honest:** with the embedder equalized, nox-mem **outperforms mem0 on both** datasets (LME +0.119, LoCoMo +0.055; 95% CIs disjoint) and all 5 represented query categories — the mem0 LoCoMo win was substantially an OpenAI-embedder effect. **Three residual confounds declared** (§6.3.2): (a) mem0's runtime version was never recorded (the artifact stores the declared pin, not `mem0.__version__`) — and if 2.0.x ran, `user_id`/`limit` were silently swallowed by `**kwargs`; (b) vector backend faiss→Chroma; (c) sample scope n=100→2,482. A fourth potential confound — (d) task-type asymmetry favoring nox (Gemini RETRIEVAL_* vs none) — was **tested by ablation (2026-06-30) and neutralized**: re-running nox with a generic embedding (no task type, matching mem0) drops it only 0.5013→0.4979 overall (−0.34 pp) and it still outperforms mem0 on overall, both datasets (LoCoMo 0.4920 vs 0.4407; LME 0.5215 vs 0.4061) and all 5 categories — the win is architectural, not an embedding-mode artifact (rigor caveat: generic corpus 99.03% gold coverage, a handicap that only lowers nox). So rc4 is "same embedding model+dim, mem0 at current release+backend" — an embedding-**matched** comparison, not a pure architecture isolation. Detail: [`paper §6.3.2`](../paper/paper-tecnico-nox-mem.md) · raws `eval/q4-comparison/output/rc4/` · ablation `output/rc4-ablation/`.
