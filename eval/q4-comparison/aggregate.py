...
def score_system(payload: dict[str, Any], k: int) -> dict[str, Any]:
    ...
    for q in queries:
        ...
        if not gold:
            # No gold for this query — skip ranking metrics (latency still counted)
...
def render_markdown(summary: list[dict[str, Any]], k: int) -> str:
    ...
    # Headline table
    out.append("## Headline\n")
    out.append("| System | nDCG@10 | R@10 | MRR | p50 (ms) | p95 (ms) | p99 (ms) | Errors |")
    out.append("|---|---:|---:|---:|---:|---:|---:|---:|")
    for s in summary:
        o = s["overall"]
        lat = s["latency_ms"]
        out.append(
            f"| **{s['system']}** | {o['ndcg@k']:.4f} | {o['recall@k']:.4f} | "
            f"{o['mrr']:.4f} | {lat['p50']:.1f} | {lat['p95']:.1f} | "
            f"{lat['p99']:.1f} | {s['n_errors']} |"
        )
