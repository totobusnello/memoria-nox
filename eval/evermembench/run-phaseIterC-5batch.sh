#!/bin/bash
# Phase IterC (Q3 POC) 5-batch SEQUENTIAL launcher.
# Mirrors run-sequential-phaseMQ.sh shape; sequential per task constraint
# (Self-Ask adds shared LLM cost surface; cleaner methodology).
#
# Strategy:
#   - Source Phase H v2 winning DBs (pre-warmed + vectorized)
#   - Clone each into $RUN_DIR
#   - Run batches 004, 005, 010, 011, 016 SEQUENTIALLY
#   - Port 18880+ (isolated from MQ @ 18842, MAP @ 18841, KG-dense @ 18843)
#
# Usage:
#   WORK=/tmp/q3-iterC-poc-<short-uuid> bash run-phaseIterC-5batch.sh
#
# Prereqs:
#   - $WORK/run-batch-phaseIterC.sh present + executable
...
PIPELINE_BAK="$WORK/pipeline.yaml.bak.iterc"
...
echo "[SEQ-ITERC] === installing pipeline.yaml (gpt-4.1-mini backbone) ==="
if [ ! -f "$PIPELINE_CFG" ]; then
    echo "[SEQ-ITERC] ERROR: $PIPELINE_CFG missing — harness not installed?"
    exit 1
fi
cp "$PIPELINE_CFG" "$PIPELINE_BAK"
cp "$WORK/phaseH-pipeline-v2.yaml" "$PIPELINE_CFG"
echo "[SEQ-ITERC] backed up original -> $PIPELINE_BAK"
echo "[SEQ-ITERC] active answer.model = $(grep -A1 '^answer:' $PIPELINE_CFG | tail -1 | tr -d ' ')"

restore_pipeline() {
    if [ -f "$PIPELINE_BAK" ]; then
        cp "$PIPELINE_BAK" "$PIPELINE_CFG"
        echo "[SEQ-ITERC] restored original pipeline.yaml"
    fi
}
...
for BATCH in "${BATCHES[@]}"; do
  ...
  if [ ! -f "$SRC_DB" ]; then
    echo "[SEQ-ITERC] WARN: $SRC_DB missing — skipping batch $BATCH"
    continue
  fi
  RUN_DIR="/root/.openclaw/evermembench-runs/phaseIterC-$BATCH-$(date +%s)"
  mkdir -p "$RUN_DIR"
  ...
  echo "[SEQ-ITERC] === starting batch=$BATCH port=$PORT run=$RUN_DIR ==="
  RUN_DIR="$RUN_DIR" WORK="$WORK" bash "$WORK/run-batch-phaseIterC.sh" "$BATCH" "$PORT" \
    > "$RUN_DIR/stream.log" 2>&1
  rc=$?
  echo "[SEQ-ITERC] batch=$BATCH exited rc=$rc"
  if [ "$rc" -ne 0 ]; then
    echo "[SEQ-ITERC] tail of stream.log:"
...
echo "[SEQ-ITERC] === aggregate per-batch analysis tails ==="
for RUN_DIR in "${RUN_DIRS[@]}"; do
  echo "--- $RUN_DIR ---"
  tail -5 "$RUN_DIR/analysis.txt" 2>/dev/null || echo "  (no analysis.txt)"
done

echo "[SEQ-ITERC] === DONE (rc=$RC_TOTAL) ==="
exit $RC_TOTAL
