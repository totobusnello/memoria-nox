...
# arxiv:2212.10496). When --hyde is passed, a hypothetical answer/passage
# is generated by an LLM and used as the dense-retrieval query (either pure
# or RRF-combined with the raw query for hybrid mode). Reuses /api/search.
# ---------------------------------------------------------------------------
...
PHASE_HYDE_PROMPT = (
    ...
    "against long-form conversation memory (Hypothetical Document "
    "Embeddings — Gao et al. 2022, arxiv:2212.10496).\n\n"
    "Write a concise 80-120 word DECLARATIVE passage that plausibly "
    "answers the question below as if quoting the relevant memory. Use "
    "the SAME language as the question.\n\n"
    "Rules: declarative statements only; mention plausible names, dates, "
    "places, and relationships even if invented (surface form matters, "
    "factual accuracy does not); do NOT prefix with 'Hypothetical answer:' "
    "or any framing — just the passage.\n\n"
    "Question: {query}\n\n"
...
def hyde_generate_hypothetical(
    ...
    """Synchronous HyDE generator using the OpenAI-compat chat completions API.

    Returns (hypothetical_text_or_None, elapsed_ms, error_or_None).
    On any failure returns (None, ms, error) — caller must fall back to
    raw-query retrieval gracefully.
    """
...
def hyde_rrf_merge(
    ...
    """RRF-union N rank-ordered hit lists into one deduped list.

    Dedup key precedence: chunk_id | id | rowid | hash(text).
    Score = sum over lists it appeared in of 1 / (rrf_k + rank).
    """
    score_by_key: dict = {}
    first_by_key: dict = {}
...
def run_conversation(
    ...
    try:
        ...
        for qi, qa in enumerate(qa_subset):
            ...
            try:
                # Retrieve — HyDE path or baseline path
                hyde_used = False
                hits = []
                sms = 0.0
                serr = None
                if hyde_enabled and hyde_api_key:
                    ...
                    if herr is not None or not hypo:
                        ...
                        if hyde_debug:
                            progress_log(
                                f"[HyDE] qa={qa.qa_index} fallback: {res.hyde_error}"
                            )
                    else:
                        ...
                        retr_t0 = time.time()
                        if hyde_hybrid:
                            # Run raw + hypothetical in series (urllib sync),
                            # take per-query top_k, RRF-union to final top_k.
                            hits_raw, ms_raw, err_raw = search_api(
                                api_base,
                                qa.augmented_question,
                                top_k,
                                DEFAULT_SEARCH_TIMEOUT,
                            )
                            hits_hyde, ms_hyde, err_hyde = search_api(
                            ...
                            # Pure HyDE — single search call on hypothetical only
                            hits, sms_pure, serr = search_api(
                                api_base, hypo, top_k, DEFAULT_SEARCH_TIMEOUT
                            )
                            sms = (time.time() - retr_t0) * 1000.0
                            res.hyde_retrieve_ms = sms
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--hyde", action="store_true",
                   help="enable Phase HyDE (Hypothetical Document Embeddings — "
                        "Gao et al. 2022, arxiv:2212.10496). Generates a "
                        "hypothetical passage via LLM and uses it as the dense "
                        "retrieval query. Default hybrid mode: RRF-merges raw + "
                        "hypothetical results.")
    p.add_argument("--hyde-pure", action="store_true",
                   help="when combined with --hyde, use hypothetical-only "
                        "retrieval (skip raw-query RRF merge). Disabled by default.")
    ...
    p.add_argument("--hyde-max-tokens", type=int, default=DEFAULT_HYDE_MAX_TOKENS,
                   help=f"HyDE generator max_tokens (default {DEFAULT_HYDE_MAX_TOKENS})")
    p.add_argument("--hyde-timeout", type=int, default=DEFAULT_HYDE_TIMEOUT_S,
                   help=f"HyDE generator timeout sec (default {DEFAULT_HYDE_TIMEOUT_S})")
    p.add_argument("--hyde-debug", action="store_true",
                   help="log HyDE generation status per QA")
