...
import { TelemetryCollector, collector, handleObsTelemetry, parseWindowParam, _internals, } from "../lib/telemetry-collector.js";
...
function ev(ts, latency_ms, opts = {}) {
    return {
        ts,
...
describe("ring buffer rollover", () => {
    test("buckets older than 24h are evicted on next record", () => {
        ...
        assert.ok(c._bucketCount() <= MAX_BUCKETS, `bucket count ${c._bucketCount()} should be <= MAX_BUCKETS ${MAX_BUCKETS}`);
    });
...
describe("concurrent writes (sequential simulation)", () => {
    test("many records in same bucket accumulate correctly", () => {
        ...
        assert.ok(Math.abs((res.aggregate.avg_latency_ms ?? 0) - 124) <= 1, `avg latency should be ~124, got ${res.aggregate.avg_latency_ms}`);
    });
...
describe("query window filter", () => {
    test("window=6h excludes events older than 6h", () => {
        ...
        assert.ok(res.aggregate.avg_latency_ms !== null &&
            Math.abs(res.aggregate.avg_latency_ms - 100) < 5, "avg should reflect only the 2h-ago event");
    });
