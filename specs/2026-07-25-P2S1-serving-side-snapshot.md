...
- [~] **T2b** 🔀 **PR ABERTO 2026-07-25** — `brief_id TEXT` no `brief_log` + migração idempotente: [nox-workspace#27](https://github.com/totobusnello/nox-workspace/pull/27). Aguarda merge + rebuild/restart na VPS. Sem isso não há como atribuir slot a brief quando dois coincidem no mesmo segundo (~9% hoje) — e o desfecho do estudo é medido por brief.
  - ⚠️ **Descoberto ao implementar: o build do `tools/nox-mem` já está VERMELHO na main** — `npm run build` sai com exit 2 e 31 erros de TypeScript (26 em `__tests__`, 3 em `export.example.ts`, 1 em `answer.example.ts`, 1 real em `src/index.ts:279` — `previewMergeEntities` não existe no módulo `knowledge-graph`). Não há `noEmitOnError`, então o `tsc` **emite mesmo com erro** e o `dist` fica em dia — por isso passou despercebido. **Consequência para o estudo: a suíte de testes não serve de gate hoje.** Precisa ser verde antes de T3/T6, senão não há como afirmar que o serving split não regrediu nada.
