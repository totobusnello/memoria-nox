...
- **Independent outcome adjudication:** blind to arm; ≥1 **external auditor** (another institution) signs that the `raw-trace → action → outcome → failure` pipeline is deterministic and specified before analysis.
