...
| **`sensibilidade-da-designacao.py`** | gera K designações alternativas pela MESMA regra sobre a MESMA população, variando só a seed (família derivada de frase fixa no script — escolher seed à mão permitiria pescar, já que o beacon é público), e no modo `--resultados` consolida a distribuição do teto. 4 guardas, todos testados por mutação: âncora (reproduz a designação publicada), sha256 da designação × manifesto, procedência idêntica entre braços, e frase do manifesto | teto **4,86% – 7,43%** em 9 sorteios, mediana 5,71%. A designação em vigor é a **menor das nove**. Ver `../CEILING-DESIGNATION-SENSITIVITY-2026-08-28.json` |
| **`replay-oportunidade.mjs --modo canal`** | **atribuição diferencial por canal**, que o `brief_log` não registra: roda o mesmo estado com `freshSlots` de produção e com 0, e diferencia. O que some ao desligar é o que o canal entregou — sem reconstruir pool nenhum. Guarda: se desligar não mudar nada em estado nenhum, o veredito vira `NAO MEDIDO`, nunca "o canal não serviu" | 40 briefs de 29/08: **80 slots** (2 por brief), **62 chunks distintos**, **todos do lote de 21–22/08**. Refuta a hipótese de que o pool principal servia tudo. Ver `../CHANNEL-ATTRIBUTION-2026-08-29.json` |
| **`pool-elegivel.py`** | reconstrói o predicado do canal de cobertura a partir do fonte (dois padrões de caminho, piso `imp OR pain`, janela de idade) e mede quanto do pool é servido por dia. Substitui a explicação de lote-e-expiração que a predição de 29/08 refutou. Guardas: dia sem brief aborta (ausência de medição ≠ cobertura 0%), e servido > elegível aborta (predicado divergiu do código) | pool de **108 de 67.187 (0,161%)**, **zero** nunca-servidos, **100% servido todo dia**, 12,4 slots por candidato. Ver `../POOL-ELEGIVEL-2026-08-28.json` |
| **`ciclo-do-lote.py`** | mede o ciclo de vida de um lote de ingestão no canal de cobertura, por dia: quantos do lote foram servidos e a idade **máxima** servida — é a máxima que localiza o corte, não a mínima. Guarda: nenhum dia pode servir algo com idade ≥ o corte declarado (mutação com `--corte 6.5` acusa 2 dias e sai `exit=1`) | lote 09–10/08: 8 dias de serviço, máxima **nunca alcança 7,00** (para em 6,95), encolhe 75→54 em 16/08 e **zera em 17/08 sem voltar** — apesar de 23 dias de elegibilidade global restantes. Ver `../BATCH-CYCLE-2026-08-28.json`. ⚠️ **Mede a UNIÃO dos canais** — `brief_log` não registra origem, e o pool principal não tem filtro de idade. Para o canal de cobertura, usar `pool-elegivel.py` |
| **`empates-por-granularidade.py`** | mede a EXPOSIÇÃO a desempate não declarado: quantos pares o comparador `(last_served, salience)` deixa indistinguíveis em cada resolução. Guardas: pares não podem diminuir ao engrossar (coarsening só funde), e pool vazio aborta em vez de reportar 'zero empates' | salience assume **6 valores** entre 108 candidatos; pares indistinguíveis **68 → 1.656** (1,18% → 28,66%), maior bloco 4 → 42. Mínima na resolução de produção. Ver `../TIEBREAK-EXPOSURE-2026-08-29.json` |
