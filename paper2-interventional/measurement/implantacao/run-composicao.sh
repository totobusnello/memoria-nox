...
if [ "$OUT_P2" != active ]; then
  ...
  L_P2="GREEN p2-composicao-do-canal motivo=ensaio-encerrado-dose-desligada outcome=${OUT_P2:-vazio} ts=$TS_P2"
  echo "$L_P2"
  printf '%s\n' "$L_P2" > /var/lib/nox-mem/p2/status-composicao.txt
  printf '{"ts":"%s","tag":"p2_gatilho_composicao","estado":"GREEN","motivo":"ensaio-encerrado-dose-desligada","outcome":"%s"}\n' \
