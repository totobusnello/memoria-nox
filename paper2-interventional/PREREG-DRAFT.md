...
**Adjudication panel (replaces human adjudicators; §0b).** Episodes are judged by an **odd-sized panel of LLMs drawn from distinct training families** — **5 families, model ids resolved 2026-07-26: Anthropic `claude-opus-5` · OpenAI `gpt-5.6-sol` · Zhipu `glm-5.2` · Moonshot `k3` (kimi-code 0.29.1) · xAI `grok-4.5`** — all five reachable from the author's environment; exact version pins remain **[TO LOCK]** at registration — chosen so that correlated annotator bias is mitigated by provenance diversity rather than by assumed impartiality. Protocol:
