...
>
> **The same four claims survived the correction to §2 in other places, and I twice miscounted how many.** Three live **outside** this document. Others live **inside** it, in the dated blocks of §9's archaeology, which the first accounting missed by looking only where the live text lives, and a second pass found one more after that. Each now carries a marker naming the band it was written under. ⚠️ **A count in prose is itself a computed claim** — it was wrong twice here, for exactly the reason this whole note is about — so this sentence no longer states one; `claims_check.py` is what enumerates, and it does so by running rather than by remembering. The three outside, and correcting §2 alone would have left them: the deposit's own front page (`DEPOSIT-README.md` in the repository, `README.md` in the deposit — rewritten), `LINK-FEASIBILITY-2026-08-15.md` (a dated measurement — given a superseded-band header and a table of what is now false, rather than rewritten, since a dated artifact that silently acquires today's numbers is worse than one plainly out of date), and one narrative line in `REACHABILITY-2026-08-16.md` (marked as describing the band then in force). **The fix for the class is not to read more carefully.** `claims_check.py` is deposited with this version: it recomputes all thirteen band-dependent quantities from the frozen constants and then sweeps the package for the superseded phrasings, failing on any occurrence outside a declared allowlist of dated records. Its own limits are stated in its docstring — the allowlist is per file, so a new stale claim inside this document would pass the sweep and is caught only if it is one of the thirteen.
>
> **(1c) A registered rule that could not run, found the same way the other three were — and the argument was already in this document.** §2's scope bullet said *"High-stakes sessions always get control and are excluded; the exclusion is arm-independent by construction."* It cannot: the allowlist classifies **actions** (`sig_coarse`, whose shares in that table are shares of actions), and briefs are composed at **session start**, before the session's actions exist. No session-level stakes classifier is defined anywhere in this registration, and `sig_coarse` occurs in it exactly once — in the bullet that depends on it. Restated as an **analytic** exclusion: every session in a treatment epoch is served the treatment brief, and opportunities in excluded classes are dropped from the outcome by a filter on the executed action's signature that cannot see the arm. Appendix C's ethics clause rested on the old reading and is corrected in the same pass, to the weaker claim that is the true one: **no high-consequence action drives a result**, not that none followed a boosted brief.
>
> ⚠️ **This is the sting worth recording.** The identical argument — *briefs are composed at session start; an opportunity is an executed action* — was written into §2 earlier the same day, to fix the designation rule. It was not carried two subsections forward to the scope rule, in the same document, in the same sitting. And the failed sentence ends in *"by construction"*, the same phrase that failed earlier today for the same reason. **A defect class does not stay fixed where it was found**: after repairing one instance, the next move is to grep the document for every other sentence that leans on the same premise, mechanically, rather than trusting that the reasoning generalised on its own.

> **(1b) The same defect was in the code, and the guard did not see it at first.** `reachable_share.py` — the instrument that produced every reach number in this package — carried `DOSES = (0.5, 1.0, 2.0)` under the comment `# the locked band` for a day after the band moved, and `--politica` defaulted to `recente`, which is not the registered designation rule. So the script, run from the deposit with no flags, reproduced neither the band nor the policy, while its own comment asserted otherwise. **No published number is affected**: both deposited JSONs were produced with explicit flags, and the corrected defaults now equal the locked values. What was affected is the only reason to deposit a script at all — a third party reproducing the work would have got the superseded band and been told it was current, and the deposit recorded no invocation anywhere against which to notice. The exact command lines are now in `REACHABILITY-2026-08-16.md` §9; runs from this version stamp a `_run` block into the JSON carrying doses, policy, replica names, cuts and input hashes, so the flags travel with the artifact rather than with a sentence in a document. `claims_check.py` gained a `cross_check` that parses the band declaration out of the script and fails if it disagrees with the one registered here — a regex could not have told the difference between a superseded tuple quoted in a correction comment and a live one, and the first version of the sweep, which looked only for the set notation `{0.5, 1.0, 2.0}`, walked past the tuple form entirely.
>
...
>
> - `COALESCE(importance,0) >= 0.7 OR COALESCE(pain,0) >= 0.7` — **passes**: verified that the ingest path populates the `importance` column via `inferImportance(chunk_type)`, so the written chunk carries 0.90 (at 2026-08-17T09:14Z, 67,510 of 67,510 chunks in the store had the column populated; zero NULL — the store is live, so the denominator moves and the invariant is the zero, re-checked at 12:41Z on 66,992). Checked because, had the column been left NULL as a literal reading of the field lock suggests, S1 and S2 would fail the gate and reach would be **zero for 100% of the corpus**.
...
>
> **Every locked number survives**, because `r̂`, `p̂0` and the ICC were computed by the replay under §3's model all along. What changed is what the registration now declares: the **effective treated population is ~30% of failures** (S2 and above), the treatment acts only through the **2 coverage slots**, and reaching the modal S1 failure would need `w ≈ 6.0` — ⚠️ **all three as of 2026-08-15, under the band `{0.5, 1.0, 2.0}`. The first is now the *lowest arm's* reach, the second is a registered restriction rather than an arithmetic consequence, and the third names a dose that is *below* the current band's top of 7.5 rather than three times it. §2 carries the corrected versions** — recorded before any arm data, so widening the band later is visibly an amendment.
>
...
- **What is registered pre-hoc:** the chain hash, `T_seed_assign`, the derivation rule above, the assignment script (with its commit hash), and the balancing constraints. **What is not knowable pre-hoc:** the seed itself.
  ...
  >
  > **What was violated, precisely, and what was not — separated because they are different failures and only one of them is fatal.** The principle broken is *the artifact must exist when it is cited*: it did not, for eighteen days. The principle **not** broken is *the artifact must not be informed by outcomes*: it could not have been. The script was written before the beacon round that seeds the assignment was drawn — the draw is deliberately sequenced **after** the mechanism freeze, so that nobody builds knowing the arm order — and therefore before any arm label, any epoch, or any outcome existed. Its inputs were the design and nothing else. A reader weighing this registration should treat it as a gap in the record rather than as a channel for adaptation, and should not have to reconstruct that distinction from the timeline; it is stated here for that reason.
  >
...
- **Treatment:** identical + additive outcome-weighted term `W_OUTCOME × severity` on chunks linked to adjudicated-failure episodes, severity per the ordinal mapping in §4.1. **LOCKED 2026-07-29:** the coefficient is re-expressed as `W_OUTCOME = w × Δ_cut` — a multiple of the measured salience spread at the brief cut, not a bare constant — with sensitivity over `w` pre-registered as secondary, replacing the absolute `{0.10, 0.15, 0.20}`. **The band was `{0.5, 1.0, 2.0}` from 2026-07-29 to 2026-08-16 and is now `{2.0, 4.0, 7.5}`** — see the designation-and-band lock below; measurement showed the two lower arms could reach nothing. (Additive per the Paper-1 v3.4 lesson: multiplicative boosts are unstable.)
  ...
  > 4. Writes are restricted to the **same low-stakes allowlist** that scopes the outcome (below). ⚠️ **Corrected 2026-08-17:** this item used to conclude *"so the written population and the treated population are the same set"*, which does not hold. A write happens at adjudication time, offline and after the action, so it **can** be filtered by action class; a brief is composed at session start, so it **cannot**. The two sets coincide at the level the analysis uses — the opportunities that enter the outcome — and diverge at the level of serving, where every session in a treatment epoch is served the treatment brief.
  >
  ...
  > >
  > > **`eligible`, defined — LOCKED 2026-08-17, because it was load-bearing and undefined.** The paragraph below used the word without saying what it means, and the obvious reading is the wrong one. A chunk is `eligible` at the composition of a brief for epoch `k` iff (i) it is a failure chunk written by the outcome channel, (ii) it has never been served (`access_count = 0` — the designation pool is the never-served set, for the reason two paragraphs down), and (iii) its `episode_id` resolves to an episode whose timestamp precedes the start of epoch `k` by **at least one epoch length**. Condition (iii) is a join on `episode_id`, which is why that field is in the locked metadata table and why the outcome channel writes a `p2_verdict` row keyed on it: the predicate is evaluable at composition time without a second source.
  > >
  > > **It gates on the episode, not on the chunk, and the difference is not cosmetic.** §3's `Opportunity` gates on when the *failure episode* was written; the chunk carries the **write instant** (locked 2026-08-16), and the writer can lag the episode across an epoch boundary. Using the chunk's own age as the criterion would therefore admit chunks whose episode is too recent to have produced an opportunity — boosting for an opportunity the outcome will not count, which dilutes toward the null — or exclude chunks whose episode qualifies but whose write was late. Neither is a bias that announces itself in the data. The join costs one indexed lookup per candidate over a pool capped at 400.
  > >
...
- **Scope:** low-stakes sessions only, per pre-committed task-type allowlist — **LOCKED 2026-07-29**, defined over the `sig_coarse` taxonomy and sized against the measured action distribution:
  ...
  The excluded classes are the ones whose failure mode is irreversible state change: file writes, VCS mutation, and service/state manipulation. The two middle classes are admitted because excluding them would remove **44.8% of all actions** and leave the study without power — but they are heterogeneous (`execucao|processo` covers both `ls` and `systemctl restart`), so eligibility inside them is decided at the **`fine`** granularity, which separates `git:log` from `git:push`. ⚠️ **RESTATED AS AN ANALYTIC EXCLUSION — 2026-08-17, and this replaces a rule that could not run.** Until today this bullet said *"High-stakes sessions always get control and are excluded; the exclusion is arm-independent by construction."* That sentence is not implementable, and the reason is the one this document had already established two sections earlier for a different rule: **briefs are composed at session start, and the allowlist classifies actions.** `sig_coarse` is an action taxonomy — the shares in the table above are shares of *actions* — so a session's classes do not exist when its brief is served. Nothing in this registration defines a session-level stakes classifier, and `sig_coarse` appears exactly once in it: here. "Always get control" was therefore a serving-time guarantee resting on information that arrives after serving.

  The exclusion is **analytic**, and applies to the outcome rather than to the brief:

  - **Serving.** Every session in a treatment epoch receives the treatment brief. There is no per-session gate, because there is nothing to gate on at session start.
  - **Analysis.** An `Opportunity` whose action `a` falls in an excluded class is **dropped from the outcome**, deterministically, by the `sig` the frozen pipeline (`c0abe143`) already computes for every action. It never enters `r̂`, `p̂0`, the ICC, or any hypothesis test. This costs no power that was ever counted: the replay that produced every locked number operates on opportunities, and excluded-class opportunities were never among them.
  - **Arm-independence.** This is now genuine rather than asserted: the filter reads `sig(a)`, a property of the executed action, and has no access to the epoch's arm. The same action is excluded in both arms or in neither.

  What is lost, stated rather than absorbed: the study can no longer claim that a high-consequence action never *followed* a boosted brief. It can claim that no such action contributes to a result. Those are different guarantees and the weaker one is the true one — see Appendix C, corrected in the same pass.
...
> | `w = 2.0` | 0.0215 | 12 | **7** |
>
> ⚠️ **The three doses in the table above are `{0.5, 1.0, 2.0}` — the band locked on 2026-07-29 and replaced on 2026-08-16.** "The locked doses" below means *locked when this was measured*. Under `{2.0, 4.0, 7.5}` the mechanism is not merely alive but reaches considerably further; §2 carries the current figures. The block is left as written because it is the record of what was known on 2026-08-15.
>
...
- the **outcome** is restricted to a pre-registered **low-stakes allowlist** (§9.4): actions whose failure mode is irreversible state change — file writes, VCS mutation, service and state manipulation, 12.2% of the corpus — are dropped from every estimate, in both arms, by a filter that reads the executed action's signature and cannot see the arm. ⚠️ **Corrected 2026-08-17.** This clause used to read *"the treatment **arm** is restricted to a pre-registered low-stakes allowlist, so high-consequence actions never vary by arm"*, and that was a stronger claim than the design supports: the brief is composed at session start, before any action exists, so a session that later executes an excluded action was served whichever brief its epoch's arm dictates. **The honest bound is that no high-consequence action drives a result, not that none followed a boosted brief.** What actually bounds the operational risk is narrower and worth stating on its own: the treatment changes **which memory the agent is shown**, never what it is permitted to do — no tool, no permission, no guardrail varies by arm — and the dose is a salience increment of at most `7.5 × 0.043` applied to two of ten brief slots;
