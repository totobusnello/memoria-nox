...
| `N_epochs` = **234** · "powered only for effects ≥ 30%" · dimensionado no **limite superior** do ICC, com efeito de desenho de **cluster desigual** (`cv²` = 0,3833) | ✅ 17/08 — **emenda de correção de erro**. O lock de 15/08 (174) usava `DE = 1 + (m̄−1)×ρ`, que pressupõe clusters de tamanho igual; os epochs vão de 1 a 115 sessões, então a fórmula subestimava o efeito de desenho e **sub-dimensionava** o estudo — a única direção que o lock (b) de 30/07 proíbe. Alocação **117 / 39 / 39 / 39**, exata. Emendado antes de qualquer epoch, de qualquer braço e do round do beacon que semeia a designação |
...
| [`LINK-FEASIBILITY-2026-08-15.md`](LINK-FEASIBILITY-2026-08-15.md) | Não há chave de junção. Duas construções caem, uma funciona — pelos 2 coverage slots. ⚠️ Os **~30%** que este documento cita são o *share* de falhas S2-e-acima, não o alcance: em `w = 2.0` o alcance medido é **58,27%**, e no topo da banda vigente é 100% |
