...
export function resolveRetentionDays(
  ...
  // V8 spec (retention.ts): NULL retention_days is an explicit never-decay marker,
  // not "unset". Only fall back to the chunk_type table when the column is truly
  // absent (undefined) — otherwise chunk_types missing from DEFAULT_RETENTION_BY_TYPE
  // (e.g. 'other', 'digest') would silently decay despite being marked never-decay.
