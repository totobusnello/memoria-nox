...
# Attribution: how many of those starts did health-probe.sh cause? Every probe
# restart appends `<epoch> <iso> <reason> http=<code> rc=<rc>` to the ledger.
# Without this the report can only guess "probe likely broken again" — the same
# sentence fires for a systemd crash-loop, an OOM kill, and a manual deploy.
PROBE_RESTARTS=$( { awk -v cutoff="$(( $(date +%s) - 3600 ))" \
    '$1 ~ /^[0-9]+$/ && $1 >= cutoff' "$API_RESTART_LEDGER" 2>/dev/null | wc -l | tr -d ' '; } || echo "?" )
case "$PROBE_RESTARTS" in ''|*[!0-9]*) PROBE_RESTARTS="?" ;; esac
PROBE_LAST_REASON=$( { awk -v cutoff="$(( $(date +%s) - 3600 ))" \
    '$1 ~ /^[0-9]+$/ && $1 >= cutoff {r=$3" "$4" "$5} END {print r}' "$API_RESTART_LEDGER" 2>/dev/null; } || echo "" )
API_CIRCUIT=$([ -f "$API_CIRCUIT_FILE" ] && echo OPEN || echo closed)
...
if [ "$RESTARTS" = "?" ]; then
    ...
    # Split probe-driven restarts from self-restarts (systemd Restart=on-failure,
    # OOM kill, manual deploy). They need opposite responses: the first means the
    # probe is trigger-happy, the second means the process is dying on its own.
    if [ "$PROBE_RESTARTS" = "?" ]; then
        OTHER="?"
    else
        OTHER=$(( RESTARTS - PROBE_RESTARTS ))
        [ "$OTHER" -lt 0 ] && OTHER=0
    fi
    ATTRIB="probe ${PROBE_RESTARTS} / self ${OTHER}"
    [ -n "$PROBE_LAST_REASON" ] && ATTRIB="${ATTRIB}; last probe reason: ${PROBE_LAST_REASON}"

    if [ "$RESTARTS" -gt 2 ]; then
        RED=$((RED+1))
        DETAILS="${DETAILS}\n🔴 nox-mem-api restarted ${RESTARTS}x in last hour (${ATTRIB})"
    else
        YELLOW=$((YELLOW+1))
        DETAILS="${DETAILS}\n🟡 nox-mem-api restarted ${RESTARTS}x in last hour (${ATTRIB})"
    fi
...
if [ "$API_CIRCUIT" = "OPEN" ]; then
    RED=$((RED+1))
    DETAILS="${DETAILS}\n🔴 nox-mem-api probe circuit OPEN — probe stopped restarting it, manual intervention needed (rm ${API_CIRCUIT_FILE})"
fi
...
BODY="${HEADER}\n\`\`\`\nchunks embedded  : ${EMBEDDED}/${TOTAL} (orphans: ${ORPHANS})\nprocedures       : ${PROCEDURES}\nreflect hits     : ${CACHE_HITS}\nrestarts 1h      : ${RESTARTS} (probe ${PROBE_RESTARTS})\napi circuit      : ${API_CIRCUIT}\ngemini 429 24h   : ${RATELIMITS}\ntrigger active   : $([ "$TRIGGER_OK" = "1" ] && echo yes || echo NO)\ncanary           : ${CANARY_LINE}\nnightly errors   : ${NIGHTLY_ERRORS}\n\`\`\`"

if [ -n "${DETAILS}" ]; then
    BODY="${BODY}\n**Details:**${DETAILS}"
fi

log "summary: red=${RED} yellow=${YELLOW} embedded=${EMBEDDED}/${TOTAL} restarts=${RESTARTS} probe_restarts=${PROBE_RESTARTS} api_circuit=${API_CIRCUIT} rl=${RATELIMITS}"
