...
### Abstract (paste verbatim — ~287 words; verify char count against the arXiv field limit before submit)
...
We present nox-mem, an open-source hybrid memory layer that combines FTS5 keyword retrieval, sqlite-vec dense retrieval, and Reciprocal Rank Fusion (RRF) over a zero-dependency SQLite database. We introduce a pain-weighted salience formula -- a weighted-additive score W_IMPORTANCE*importance + W_RECENCY*recency + W_PAIN*pain + W_ACCESS*access (weights 0.55/0.15/0.10/0.20) -- where pain encodes incident severity (0.1 trivial to 1.0 production outage) as a design signal to keep high-stakes memories retrievable when recency is low (isolated effect directional, not yet significant -- §7.1). We further propose a Conditional Hard Mutex (G10d), which gates section and source-type boosts on query entity count (threshold t=2), recovering multi-hop (+1.58% nDCG@10) and adversarial (+3.04% nDCG@10, +6.25% MRR) regressions, at the cost of moderate single-hop dilution.
