...
#### 6.3.2 Controlled-embedding variant (rc4 — all-Gemini, full eval)

The §6.3 split is reported under each system's *native default* embedder (the "as-configured" stance, §6.5.5). Because nox-mem defaults to Gemini 3072d and Mem0 to OpenAI 1536d, the most obvious confound on the split is the embedding provider itself. To isolate it we ran a controlled variant — **rc4** — in which both systems use the **same embedder**: `gemini-embedding-001` at 3072d (the model and dimensionality nox-mem runs in production), with Mem0 re-pointed to it (zero adapter edit; embedder injected via config). This variant also replaces the n=100 sample with the **full evaluation set (n=2,482: 1,982 LoCoMo + 500 LongMemEval)** over the full 6,822-chunk corpus, scored identically to §6.3 (binary-relevance nDCG@10, k=10, same gold encoding; gold-match coverage verified at 100% on both datasets).

| System (all-Gemini 3072d, n=2,482) | LongMemEval nDCG@10 | LoCoMo nDCG@10 | Overall |
|---|---:|---:|---:|
| **nox-mem** (hybrid FTS5 + Gemini + RRF) | **0.5255** | **0.4952** | **0.5013** |
| Mem0 (Gemini embedder, Chroma) | 0.4061 | 0.4407 | 0.4337 |

**Result — the split does not survive embedding control.** With the provider equalized, **nox-mem leads both benchmarks** (LongMemEval +0.119, LoCoMo +0.055 nDCG@10) and **all five represented query categories** (§6.4). The Mem0 LoCoMo win in §6.3 was therefore substantially an embedder effect — OpenAI 1536d on LoCoMo — rather than a retrieval-architecture advantage: once both systems embed with the same Gemini model, nox-mem's hybrid FTS5 + dense + RRF retrieval leads on LoCoMo as well.

**Residual confounds — declared, not a clean architecture isolation (per §6.6).** Two factors prevent attributing the inversion purely to the embedding. **(a) Mem0 version drift:** the Mem0 client changed major version between the canonical run and rc4 (0.1.x → 2.0.10; its `search`/`get_all` API changed, requiring a compatibility fix on our adapter), so rc4's Mem0 (0.4337) is *not* the same build as §6.3's Mem0 (0.4686 LoCoMo) — part of the gap may be the version, not the embedder. **(b) Task-type asymmetry favoring nox-mem:** nox-mem passes Gemini's `RETRIEVAL_DOCUMENT`/`RETRIEVAL_QUERY` task types (retrieval-optimized embeddings); Mem0's embedder does not — same model and dimensionality, but nox-mem uses the model in its retrieval-tuned mode. We did not patch Mem0's embedder call (it would breach the no-adapter-edit rule). rc4 is therefore best read as *"same embedding model and dimensionality, Mem0 at its current release"* → nox-mem leads — **not** as a surgical architecture-only isolation; a fully controlled study (matched Mem0 version + matched task type) is left to follow-up. (Mem0 also lost 4 of 6,822 chunks to transient Gemini 503s on ingest; this affects one query, whose gold chunk Mem0 fails to retrieve in top-10 even when the chunk is present — verified zero aggregate effect.) Raw results and aggregator output are published at `eval/q4-comparison/output/rc4/`.

### 6.4 Per-category breakdown

Per-query-category breakdown from the **rc4 controlled-embedding run** (§6.3.2; all-Gemini 3072d, n=2,482 across both datasets). The canonical 2026-06-15 run reported only dataset-level metrics, so the breakdown is populated from rc4, where per-category gold labels (the datasets' native question types, mapped to six canonical buckets) are wired into the harness. Only nox-mem and Mem0 participate: agentmemory's server-side embedder cannot be re-pointed to Gemini, and the three §6.3.1 gaps likewise do not run under the controlled embedder.
...
The §6 Q4 COMPARISON is pre-registered (`specs/2026-05-23-Q4-comparison-execution-plan.md`). After a methodology smoke (2026-05-24, n=20) and an infrastructure abort (incident D76, §7.1 L5), the **canonical run executed 2026-06-15** (n=100/dataset, 6 systems: 3 produced numbers, 3 documented gaps — §6.3), and the **all-Gemini controlled variant (rc4) executed 2026-06-29** (full n=2,482 — §6.3.2 / §6.4). With the embedding provider equalized, nox-mem leads both datasets and all five represented categories, the as-configured split (§6.3) and controlled result (§6.3.2) reported side by side per §6.6, residual confounds declared. The D43 gate (top-3 on ≥2 of the 4 key metrics) is satisfied, unlocking the GTM Phase 2 pre-launch defense.
