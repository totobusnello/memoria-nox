...
> ⚠️ **Scope of this subsection (2026-09-10).** The non-runs above are non-runs *of the 2026-06-15 canonical pod*. Zep has since been brought up on a host with a working Docker daemon and a paid key and is being re-run for §6.3; when its numbers land, Zep leaves this list and the count of non-running competitors drops from three to two. The operational finding that survives is the one verified in source rather than inferred from our environment: Zep cannot boot without a paid LLM key (§6.8).
