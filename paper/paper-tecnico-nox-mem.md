...
[^nox-mem-rss]: The `~341 MB RSS` figure for nox-mem in Table 2 is **measured in prod** on 2026-05-24 (VPS 187.77.234.79, 6830 chunks live, uptime 9h28min). `ps -o rss=` returned 349,276 KB (≈341 MB); `systemd` cgroup `MemoryCurrent` was ~727 MB. The ~386 MB delta between RSS and cgroup is SQLite memory-mapped I/O (chunks table, FTS5 index, vec0 index) — kernel-managed page cache, reclaimable on memory pressure, not exclusive process memory. Standard `ps`/`top` RSS is the canonical comparison metric used in this table. EverOS ~4 GB+ figure is from the EverMind-AI docker-compose defaults; at ~12× the nox-mem RSS figure, the directional advantage is unchanged from the original estimate.
