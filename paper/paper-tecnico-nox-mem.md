...
- **EverMind-AI / EverOS** — runs only as an HTTP service and requires two third-party credentials that were **not available to the 2026-06-15 run** (OpenRouter for LLM extraction + DeepInfra for embedding/rerank, the latter provider-specific); installation also requires authorizing an external-repo `pip install -e`. Out of scope for that time-box. **That barrier was removed later, and the ingest half is measured** — see below; the retrieval half is not reported in this revision.
...
**No retrieval quality number for EverOS appears in this revision.** The sweep was running
when this section was written; a number will be reported only against a closed artifact,
and §6.9's count of systems without a number is stated as of this revision.
