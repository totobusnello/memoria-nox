...
\textbf{nox-mem v3.8 --- March 2026, §5 third revision May 2026 (Wave A
...
design}. Every retrieval and retention decision is governed by a
\texttt{salience\ =\ recency\ ×\ pain\ ×\ importance} formula (where
\emph{pain} is an operator-assigned severity in {[}0.1, 1.0{]},
persisted on every chunk), and every ranking change is gated by a
mandatory shadow-mode telemetry phase before activation in production.
Compared to existing memory systems (mem0, Letta, Zep, EverOS, LightRAG,
and MeMo), nox-mem offers several advantages: \textbf{(a)} \emph{live
...
production since March 14, 2026, the system manages 62.9k+ memory chunks
with \textasciitilde99.97\% vector coverage, \textasciitilde402
knowledge graph entities with \textasciitilde544 relations, and serves 6
specialized AI agents with isolated yet interconnectable memory spaces.
On the entity-flavored golden set (n=100), the full ablation stack
reaches \textbf{nDCG@10 = 0.6237} (+78.8\% over the G3 pre-Wave-A
baseline; §5.1.1). On the EverMemBench cross-system benchmark (5-batch,
n=3,121), nox-mem scores \textbf{62.22\%} with Gemini-2.5-flash (+2.95
pp over MemOS; §5.1.5), \textbf{51.68\%} with GPT-4.1-mini (+9.13 pp
over MemOS, 95\% CI {[}49.88, 53.49{]}; §5.1.6), and \textbf{63.28\%
Overall + 88.42\% Memory Awareness composite with Gemini-3-flash ---
both SOTA versus MemOS Table 4 baselines} (+20.73 pp Overall, +32.74 pp
MA; §5.1.10). On classical multi-hop QA, nox-mem achieves dual SOTA
without specialized fine-tuning: \textbf{MuSiQue dev F1 58.62\%} (+22.82
pp over IRCoT, +8.92 pp over EX(SA); §5.2.1) and \textbf{HotPotQA dev
distractor ans\_F1 73.37\%} (above DPR+FiD reader SOTA; §5.2.2). LoCoMo
cross-bench retrieval@10 strict reaches \textbf{74.52\% --- above Mem0's
published SOTA F1 of 66.88\%} (§5.3.1); the F1 push is rank-5 (51.85\%,
above Zep / LangMem) constrained by a verbosity gap (§5.3.2). The §5.4
resolution of the EverMemBench F\_MH paradox shows the 3--7\% absolute
is a corpus-structural property, not a multi-hop reasoning ceiling.
...
\textbf{cost \$0/query KG path and 769× cheaper than Mem0 Cloud on
hybrid} (§5.7.2), and \textbf{399 MB RSS self-hosted single-process}
footprint (§5.7.3). The cross-bench LongMemEval validation (n=300, §5.6)
confirms the same per-category fingerprint across an orthogonal
benchmark distribution. Wave 2 (2026-05-31 -- 2026-06-02,
§5.5.4--§5.5.8) contributes an empirical per-backbone × per-knob
composability study: three Wave A single-stage retrieval knobs (KG path,
AC, MQ) transfer at only 0--40\% efficiency from gpt-4.1-mini to
Gemini-3-flash (D75), establishing that retrieval-stage compensation
...
The salience function lives in \texttt{src/lib/salience.ts} \footnote{\texttt{src/lib/salience.ts}
  ...
  \texttt{salience\ =\ recency\ ×\ pain\ ×\ importance}, mirrored in the
  staged copy at \texttt{staged-1.7a/edits/salience.ts} (lines 1--80
  contain the module docstring spelling out the formula, the three-state
  mode gate, and the per-type retention defaults).} (and is mirrored in
the staged copy at \texttt{staged-1.7a/edits/salience.ts}). Its
...
\tightlist
\item
  \textbf{recency} ∈ {[}0,1{]} --- half-life-style decay over the
...
\item
  \textbf{pain} ∈ {[}0,1{]} --- severity as described in §3.4.2.
\item
  \textbf{importance} ∈ {[}0,1{]} --- \texttt{chunk\_type} /
...
\texttt{section\ ∈\ \{compiled,\ frontmatter,\ timeline\}}, recovering
...
\begin{quote}
\texttt{section\_boost\ ×\ source\_type\_boost\ (Hard\ Mutex,\ query\_entity\_count\ ≤\ 2)\ ×\ salience\ v2\ additive}
\end{quote}
...
\item
  ...
  than MuSiQue (≤4 paragraphs) or HotPotQA (2 bridge paragraphs).
\item
...
cost within budget PASS, MA composite borderline 🟡 (−3.53 pp, falls
inside the MA tolerance band but at its lower edge). Final verdict:
...
\textbf{Ceiling refinement --- D74 vs D69 / D72.} D69 cravado the Wave
A/B/C single-stage retrieval ceiling at +7.25 pp F\_MH (§5.1.9). D72 (PR
\#410, third revision) framed F\_MH as ``structural challenge of
...
\textbf{Distinction: infrastructure abort ≠ scientific failure.} The
distinction is load-bearing for interpreting this result:
...
\paragraph{5.7.2 Cost SOTA --- \$0/query KG path, 769× cheaper than Mem0
Cloud}\label{cost-sota-0query-kg-path-769-cheaper-than-mem0-cloud}
...
embedding + retrieval) & \textbf{∞ cheaper} \\
Retrieval API cost (hybrid w/ Gemini embedding) & \$0.0000013/query &
\$0.001/query & \textbf{769× cheaper} \\
...
only local SQL + regex with no LLM call. The hybrid path costs
\$0.0000013/query (Gemini-embedding-001 query at \$0.000013/1k chars ×
\textasciitilde100 chars average). The 769× factor establishes
structural cost dominance over hosted memory APIs.
...
\begin{quote}
\textbf{Status (atualizado 2026-05-24 \textasciitilde22h BRT ---
FINAL):} Sat 2026-05-24 FINAL closure. \textbf{4/6 systems com dados
reais.} Decision A aprovada: ship 4/6 (Zep NO GATED por OpenAI embedding
requirement; EverMind-AI FAIL SKIP por repo 404 confirmado PR \#281).
nox-mem headline: nDCG@10=0.6380 (Gemini hybrid) / 0.3753 (FTS5-only).
mem0 (500-chunk cap) + agentmemory (1401-chunk cap) + Letta (partial 1/5
smoke) medidos com caveats de corpus. Canonical 100-query run deferred
Sun 2026-05-25 com corpus uniforme sem cap. Princípios (§6.5),
anti-cherry-pick (§6.6) e pre-registration (§6.7) imutáveis. Nota: ``Sat
2026-05-24 partial; canonical full-corpus run Sun 2026-05-25.'' Refs:
\texttt{{[}{[}q4-real-numbers-sat-2026-05-24{]}{]}} ·
...
nox-mem em top-3 em \textgreater=2 das 4 métricas chave (nDCG@10, R@10,
MRR, latência) --- destravando a GTM Phase 2.
...
\textbf{Sat 2026-05-24 FINAL --- 4/6 systems with real data. Decision A:
ship 4/6 (Zep gated, EverMind skipped). Canonical 100-query run deferred
Sun 2026-05-25.}

\textbf{LongMemEval n=100 (canonical --- pending Sun 2026-05-25):}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{nox-mem} & \texttt{{[}pending\ Sun\ canonical{]}} &
\texttt{{[}pending{]}} & \texttt{{[}pending{]}} & \texttt{{[}pending{]}}
& \texttt{{[}pending{]}} & \texttt{{[}pending{]}} &
\textasciitilde\$0.00 (local) \\
Mem0 &
\texttt{{[}pending\ Sun\ canonical\ —\ full\ corpus,\ no\ 500-cap{]}} &
\texttt{{[}pending{]}} & \texttt{{[}pending{]}} & \texttt{{[}pending{]}}
& \texttt{{[}pending{]}} & \texttt{{[}pending{]}} &
\textasciitilde\$0.10+ ingest \\
agentmemory &
\texttt{{[}pending\ Sun\ canonical\ —\ full\ corpus,\ no\ 20\%-cap{]}} &
\texttt{{[}pending{]}} & \texttt{{[}pending{]}} & \texttt{{[}pending{]}}
& \texttt{{[}pending{]}} & \texttt{{[}pending{]}} &
\textasciitilde\$0.00 \\
Letta & \texttt{{[}pending\ —\ partial\ only;\ agent-loop\ arch{]}} &
\texttt{{[}pending{]}} & \texttt{{[}pending{]}} & \texttt{{[}pending{]}}
& \texttt{{[}pending{]}} & \texttt{{[}pending{]}} &
\texttt{{[}pending{]}} \\
Zep &
\texttt{{[}FALHA:\ NO\ GATED\ —\ OpenAI\ embedding\ requirement\ +\ adapter\ rewrite\ needed;\ deferred\ post-launch{]}}
& --- & --- & --- & --- & --- & \textasciitilde\$0.02 est. \\
EverMind-AI &
\texttt{{[}FALHA:\ FAIL\ SKIP\ —\ repo\ EverOS-AI/EverMind-AI\ returns\ 404;\ confirmed\ 2026-05-24\ PR\ \#281{]}}
& --- & --- & --- & --- & --- & --- \\
\end{longtable}
}

\textbf{LoCoMo full (canonical --- pending Sun 2026-05-25):}
...
Zep e EverMind-AI são reportadas com
\texttt{{[}FALHA:\ \textless{}razão\textgreater{}{]}} explícito em vez
de omitidas --- consistente com §6.6 (anti-cherry-pick). O run canônico
Sun 2026-05-25 atualiza as células \texttt{{[}pending{]}} para os 4
sistemas restantes com corpus uniforme (sem cap). Ref:
\texttt{{[}{[}q4-real-numbers-sat-2026-05-24{]}{]}}.
...
\item
  \textbf{Per-dataset breakdown explícita (PR \#318, 2026-05-23 ---
  rev3).} O run Gemini hybrid@500 revelou que o aggregate (0.0918)
  mascara um resultado por-dataset decisivo. Reportamos as três linhas
  explicitamente:
  ...
  0.1835 \textbf{supera} mem0@500 = 0.1315 em \textbf{+40\%} na dimensão
  de memória conversacional. O aggregate (0.0918) fica abaixo de mem0
  por um \textbf{artefato de corpus-ordering}: ao 500-chunk cap, os
  5.882 chunks do LoCoMo esgotam o cap antes de qualquer chunk do
  LongMemEval ser ingerido --- as 10 queries LongMemEval ficam com
  cobertura zero, zerando o nDCG desse dataset e puxando o aggregate
  para baixo. Hybrid stack lift sobre FTS5@500: \textbf{+97\%} (0.0466 →
  0.0918), validando o valor arquitetural do stack mesmo em corpus
  esparso.

  \textbf{H2 finding (PR \#311, mantido):} FTS5-only@500 = 0.0466 vs
  mem0@500 = 0.1315 é \textbf{real e arquitetural} para o modo FTS5-only
  --- LLM-rewriting do mem0 produz generalização semântica que FTS5
  isolado não consegue. PR \#318 mostra que o Gemini hybrid completo
  inverte esse resultado no escopo conversacional.

  \textbf{Disclosure obrigatória:} o aggregate ±0.05 está dentro do
  intervalo inconclusivo para n=20. O árbitro definitivo é o run
  canônico full-corpus (corpus uniforme sem cap, LoCoMo + LongMemEval
  completos para todos os sistemas). \textbf{Phase 2 gate usa AMBOS}
  per-dataset + aggregate no run canônico --- não apenas o número que
  favorece nox-mem.
...
nox-mem em top-3 em \textgreater=2 das 4 métricas chave (nDCG@10, R@10,
MRR, latência). Atendido o gate, GTM Phase 2 está destravada conforme
...
em \textgreater=2 das 4 métricas chave) é avaliado contra o run
canônico; o smoke valida a metodologia + confirma que nox-mem retrieval
funciona end-to-end, destravando a defesa pre-launch da GTM Phase 2.
