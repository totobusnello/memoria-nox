...
O resultado: uma lição de prod-outage de seis meses atrás supera documentação atualizada ontem sobre assunto menor. Exatamente como a memória humana funciona. O sistema roda 64.180+ chunks com essa dimensão ativa, validada por 7 dias de telemetria real.

### Shadow Discipline: nenhuma mudança de ranking entra sem sete dias de prova

`NOX_SALIENCE_MODE=shadow` é constraint arquitetural enforced via cron e `/api/health` — não uma boa prática documentada que alguém pode ignorar. Qualquer mudança que afeta ranking fica em shadow por mínimo 7 dias, comparada contra comportamento anterior, antes de ativar.
...
<!-- TRACKED CHANGES -->
<!--
1. Linha 7: "É disciplina." separado em parágrafo — "Foi nesse dia que ficou claro que o problema [...] É disciplina." → quebra após "É disciplina." transformada em frase autônoma com vírgula → "ficou claro: o problema [...] É disciplina." — RAZÃO: colon cria pausa dramática antes da tese; parágrafo solo aumenta impacto do hook final.

2. Linha 25: "E então vieram os incidents." → "Então vieram os incidents." — RAZÃO: "E então" é conjunção fraca de transição; "Então" sozinho tem cadência mais seca e urgente, alinhada ao tom de relato de incident.

3. Linha 43: "O resultado é que uma lição de prod-outage" → "O resultado: uma lição de prod-outage" — RAZÃO: substituição de "é que" por dois-pontos elimina verbo de ligação fraco, gera ritmo mais direto nas frases-bomba (paralelismo com seção anterior).

4. Linha 43: "O sistema agora tem 64.180+ chunks" → "O sistema roda 64.180+ chunks" — RAZÃO: "tem" é verbo de estado; "roda" é verbo de ação que evoca produção real ativa — alinhado com a claim de "produção real há 4 meses".

5. Linha 57: "Atlas pode recuperar esse contexto diretamente" → "Atlas recupera esse contexto diretamente" — RAZÃO: "pode recuperar" é condicional; "recupera" é declarativo presente — mais confiante e factual para uma claim de design.

6. Linha 109: "A comunidade científica de memória de agentes não tem ainda o conceito" → "A comunidade científica de memória de agentes ainda não tem o conceito" — RAZÃO: "não tem ainda" é ordem lusitana/informal; "ainda não tem" é a ordem natural São Paulo PT-BR e soa mais assertivo em contexto científico.

7. Linha 115: "Reviewer pode refutar" → "O reviewer pode refutar" — RAZÃO: artigo definido "O" em PT-BR formal é mandatório antes de substantivo comum em posição de sujeito; sem artigo soa como tradução literal do inglês (falso economismo).
-->
