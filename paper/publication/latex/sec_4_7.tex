...
for this corpus type. nox-mem hybrid achieves a $3.5\times$ lift over the strongest
...
\begin{table}[!ht]
  \caption{Internal corpus comparison (Corpus~A, golden queries). nox-mem hybrid 3-run mean
    is on the 50-query main set R01b; external baselines (BM25 Pyserini, multilingual-e5-base)
    are one-shot evaluations on the full 60-query set (R01b $\cup$ R01c). FTS5 vanilla and
    nox-mem hybrid columns are directly comparable ($n$=50, R01b, 3-run mean $\pm$ std).}
  \label{tab:hybrid}
  \centering
  \small
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{lrrrrr}
    ...
    \midrule
    FTS5 vanilla (SQLite BM25, this work) & 50 & $0.0123 \pm 0.0000$ & $0.0200 \pm 0.0000$ & $0.0100 \pm 0.0000$ & $0.0040 \pm 0.0000$ \\
    ...
    \textbf{nox-mem hybrid (FTS5 + Gemini 3072d + RRF, $k$=60)} & 50 & $\mathbf{0.5213 \pm 0.0004}$ & $0.4889 \pm 0.0028$ & $0.6800 \pm 0.0047$ & $0.2640 \pm 0.0000$ \\
    \midrule
    $\Delta$ (hybrid $-$ BM25 Pyserini) & --- & \textbf{$+0.3738$ ($3.5\times$)} & --- & --- & --- \\
    $\Delta$ (hybrid $-$ FTS5 vanilla)  & --- & \textbf{$+0.5090$ ($+50.9$~pp)} & --- & --- & --- \\
    \bottomrule
  \end{tabular}}
  \begin{minipage}{\linewidth}
    \vspace{4pt}\footnotesize
    nox-mem hybrid 3-run mean $\pm$ std (Runs \#10/\#11/\#12). External baselines from
    \texttt{results/E01-BM25-baseline-summary.md} (BM25 Pyserini) and
    \texttt{results/E02-E5-multilingual-baseline-summary.md} (multilingual-e5-base).
    The $n$=60 vs $n$=50 difference arises because R01c (10 held-out queries, Q51--Q60)
    was added to external baselines as a one-shot evaluation but excluded from the hybrid
    headline metric to preserve held-out integrity (\S3.8). Restricted to $n$=50 (R01b
    only), BM25 Pyserini = $0.1475$ and multilingual-e5-base = $0.3070$ are unchanged
    within rounding (per-category breakdown in \texttt{results/E01,E02}).
    The $3.5\times$ lift over BM25 Pyserini is the cited \emph{strongest external lexical
    baseline} comparison; see Table~\ref{tab:beir} for cross-corpus generalization (BEIR
    TREC-COVID).
  \end{minipage}
\end{table}
...
\begin{table}[t]
  \caption{Layer ablation on internal corpus, Corpus~A ($n$=50 golden queries, 3-run mean
    $\pm$ std). Two layer ablations are reported here; three additional ablations
...
\begin{tabular}{llll}
  ...
  \begin{minipage}{\linewidth}
    \vspace{4pt}\footnotesize
    FTS-only is confirmed from R01b (Table~\ref{tab:hybrid}). Three additional layer
    ablations are flagged for the next revision: (E7) FTS+semantic without RRF
    (\texttt{NOX\_RRF\_DISABLE=1}), (E8) hybrid without salience boost
    (\texttt{NOX\_SALIENCE\_MODE=off}), and (E9) hybrid without section boost
    (\texttt{NOX\_SECTION\_BOOST\_MODE=off}). Per-layer environment toggles are
    documented in the repository so any third party can reproduce them; results are
    deferred rather than withheld.
  \end{minipage}
\end{table}
...
during pain-distribution design or coefficient selection. Held-out integrity (\S3.8) is
preserved for the headline hybrid metric (\S5.1, $n$=50, R01b only).
...
architectural contributions. Note that the current nDCG@10 of $0.5213 < 0.6$, which is below
