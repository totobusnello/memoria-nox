...
PagerDuty P1--P5 and SRE error budgets, not psychometric scales (\S4.3, \S5.5). Ablation
($n=31$, hybrid mode) shows non-significant aggregate effect ($\Delta = +0.0065$, 95\%~CI
$[-0.014, +0.034]$); lift concentrates in tied-semantic regimes (Q55,
$\Delta = +0.349$). The binding constraint is BM25 recall ceiling: 55 of 60 queries fail
FTS-only regardless of pain calibration; full hybrid retains Recall@10 $= 0.6800$ via
Gemini. The transferable contribution is treating severity as a typed schema input; pain
belongs as a post-RRF re-ranker. (2) Shadow discipline: a seven-day gate before any
...
nDCG@10 $= 0.5213 \pm 0.0004$ ($n=50$, 3-run mean) versus 0.0123 for FTS5 vanilla and
0.1475 for BM25 Pyserini ($n=60$), a $3.5\times$ lift over the strongest lexical baseline.
...
lexical difficulty is corpus-dependent; BEIR TREC-COVID evaluation is ongoing. Comparative
positioning --- corpus scale $>100$K is the remaining open gap --- is in \S2.5. Code,
