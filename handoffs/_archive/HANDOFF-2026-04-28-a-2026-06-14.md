# HANDOFF — arquivo histórico (2026-04-28 → 2026-06-14)

> Arquivado em 2026-06-27 na sanitização do `docs/HANDOFF.md` (que ficou só com estado-vivo desde 06-15).
> Inclui ~70 entradas datadas de abril/maio/início-junho + meta-seções antigas (RETOMADA, arXiv checklist 05-02, gates, sessões granulares) + o quick-ref legado (IP <NOX_TAILSCALE_IP>, leituras de 05-02).
...
✅ Mac↔VPS: MCP-over-SSH (16 tools) + https://<NOX_TAILSCALE_HOST> (Bearer, 47-76ms)
...
> **Atualizado:** 2026-05-20 ~10h45 BRT. **VPS estava no IP novo $NOX_VPS_HOST (não <VPS_IP> — false alarm de IP swap, uptime intacto 20d). Deploy Wave A novo aplicado em prod: search.ts + salience.ts via scp + api-server.ts via sed FIND/REPLACE (3 patches). Build limpo, restart OK, /api/health.salience.mode=active, 68995/68995 vectorCoverage preservado. Gold Q87+Q88 temporal curados via PR #159 (chunks 216203+216204). 6 PRs merged em main hoje.**
...
- Antigo: `<VPS_IP>` (gone)
- Novo: `$NOX_VPS_HOST` (active, hostname `<NOX_VPS_HOST>`, uptime **20d** — não foi reboot)
...
> **Atualizado:** 2026-05-20 ~11h BRT — **5 PRs merged em main (#154-#158): SOURCE_TYPE_BOOST map cobre 11 backfill keys, visual identity sincronizada com +78.8%, paper §5 reescrito com 4-claim sub-evidence, temporal Q1 spike isolado, api-server patch documental. VPS <VPS_IP> OFFLINE 100% packet loss — deploy de todos gated até host voltar.**
...
### 🚨 VPS <VPS_IP> OFFLINE 2026-05-20 ~10h BRT
...
- VPS access: `root@<NOX_TAILSCALE_IP>` (Tailscale). Snapshot pré-op em `/var/backups/nox-mem/pre-op/` (retention 7d).
...
```bash
ssh root@<NOX_TAILSCALE_IP> 'tail -3 /var/log/nox-mem/beir-progress.log && tmux ls'
```
...
```bash
ssh root@<NOX_TAILSCALE_IP> 'curl -s http://127.0.0.1:18802/api/health | jq "{total: .chunks.total, embedded: .vectorCoverage.embedded, dbMB: .dbSizeMB}"'
ssh root@<NOX_TAILSCALE_IP> 'tail -5 /var/log/nox-seh-report.log'
```
...
- Backups de antes do split em `archive/pre-split-20260501/`
...
```bash
ssh root@<NOX_TAILSCALE_IP> '/root/bin/improvements check'
```
...
ssh root@<NOX_TAILSCALE_IP> 'cd /root/.openclaw/workspace/tools/nox-mem && node --test dist/__tests__/retention.test.js dist/__tests__/op-audit-e2e.test.js 2>&1 | tail -5'

# OpenClaw release watcher state
ssh root@<NOX_TAILSCALE_IP> 'cat /root/.openclaw/upgrade-watcher/state.json'

# Latest checkpoint
ssh root@<NOX_TAILSCALE_IP> 'ckpt list | head -3'

# Logs gateway
ssh root@<NOX_TAILSCALE_IP> 'journalctl -u openclaw-gateway --since "10 min ago" --no-pager | tail -30'

# CLI nox-mem (lembrar source env primeiro)
ssh root@<NOX_TAILSCALE_IP> 'set -a; source /root/.openclaw/.env; set +a; nox-mem --help'
