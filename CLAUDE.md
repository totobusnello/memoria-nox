...
- **RelayPlane está zumbi desde pós-incident billing proxy** — service active, config tem chaves mas `ANTHROPIC_BASE_URL` não no env do gateway, gateway chama `api.anthropic.com` direto. `data.db` com 0 runs em 7d. Budget cap ($5/dia, $1/hora, $0.50/request) está **inerte**. Decisão pendente: ressuscitar com config nova + mudar `ANTHROPIC_BASE_URL=http://127.0.0.1:4100` no systemd, ou parar o service
